...
> **Not:** Firestore'a otomatik yazım için `firebase-admin` paketi kurulu olmalı.
...
**Örnek node yapısı:**
`users/{uid}`, `tasks/{id}`, `task_participations/{id}`, `deposits/{id}`, `withdrawals/{id}`, `stats/{id}`, `ads/{id}`, `ad_views/{id}`
